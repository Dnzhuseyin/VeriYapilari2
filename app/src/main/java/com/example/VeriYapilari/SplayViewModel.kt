...
class SplayViewModel : ViewModel() {
    private val tree = SplayTree()
    var rootNode: SplayNode? by mutableStateOf(null)
        private set
    ...
    private fun findTreeBounds(node: SplayNode?): Pair<Float, Float> {
    ...
    private fun applyOffset(node: SplayNode?, offset: Float) {
        if (node == null) return
        node.x += offset
    ...
    private fun calculateTreeLayout(node: SplayNode?, x: Float, y: Float, levelWidth: Float, level: Int) {
        if (node == null) return
        node.x = x
        node.y = y
